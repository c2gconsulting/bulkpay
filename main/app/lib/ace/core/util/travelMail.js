 ...
 */
 Core.sendRetirementApprovalMail = (currentTravelRequest, TravelRequestHelper, recieverID, nextUserID, subjects) => {
  ...
  const supervisor = Meteor.users.findOne(currentTravelRequest.supervisorId) || defaultUserData;
  const budgetHolder = Meteor.users.findOne(currentTravelRequest.budgetHolderId) || defaultUserData;
  const financeApprover = Meteor.users.findOne(currentTravelRequest.financeApproverId) || defaultUserData;
  const bst = Meteor.users.findOne(currentTravelRequest.bstId) || defaultUserData;
  ...
  let bstSubject = "Please approve travel retirement for " + createdBy.profile.fullName;

  const { bstCond, financeCond, securityCond } = Core.getApprovalQueries(createdBy);
  ...
  const { 
    ...
    RETIREMENT_REJECTED_BY_BST, } = Core.ALL_TRAVEL_STATUS
  ...
  if (isHOD && currentTravelRequest.retirementStatus === RETIREMENT_APPROVED_BY_HOD){
    createdBySubject = `${HOD}: ` + supervisor.profile.fullName + " has approved your travel retirement";
    supervisorSubject = "You have approved " + createdBy.profile.fullName + "'s travel retirement";
  }

  if (isBUDGETHOLDER && currentTravelRequest.retirementStatus === RETIREMENT_APPROVED_BY_BUDGETHOLDER){
    createdBySubject = `${BUDGETHOLDER}: ` + budgetHolder.profile.fullName + " has approved your travel retirement";
    budgetHolderSubject = "You have approved " + createdBy.profile.fullName + "'s travel retirement";
  }

  if (isFINANCE && currentTravelRequest.retirementStatus === RETIREMENT_APPROVED_BY_FINANCE){
    createdBySubject = `${FINANCE}: ` + financeApprover.profile.fullName + " has approved your travel retirement";
    financeApproverSubject = "You have approved " + createdBy.profile.fullName + "'s travel retirement";
  }

  if (isBST && currentTravelRequest.retirementStatus === RETIREMENT_APPROVED_BY_BST){
    createdBySubject = `${BST}: ` + bst.profile.fullName + " has processed your travel retirement";
    bstSubject = "You have approved " + createdBy.profile.fullName + "'s travel retirement";
  }

  // REJECTION SUBJECTS
  // HOD
  if (isHOD && currentTravelRequest.retirementStatus === RETIREMENT_REJECTED_BY_HOD) {
    createdBySubject = `${HOD}: ` + supervisor.profile.fullName + " has rejected your travel retirement";
    supervisorSubject = "You have rejected " + createdBy.profile.fullName + "'s travel retirement";
  }
  // BUDGET HOLDER
  if (isBUDGETHOLDER && currentTravelRequest.retirementStatus === RETIREMENT_REJECTED_BY_BUDGETHOLDER) {
    createdBySubject = `${BUDGETHOLDER}: ` + budgetHolder.profile.fullName + " has rejected your travel retirement";
    supervisorSubject = "You have rejected " + createdBy.profile.fullName + "'s travel retirement";
  }
  // FINANCE
  if (isFINANCE && currentTravelRequest.retirementStatus === RETIREMENT_REJECTED_BY_FINANCE) {
  ...
  // BST
  if (isBST && currentTravelRequest.retirementStatus === RETIREMENT_REJECTED_BY_BST) {
    createdBySubject = `${BST}: ` + bst.profile.fullName + " has rejected your travel retirement";
    bstSubject = "You have rejected " + createdBy.profile.fullName + "'s travel retirement";
  }
  // END OF REJECTION SUBJECTS
  ...
  if (isBUDGETHOLDER || nextBUDGETHOLDER) {
    console.log('Email To: BUDGETHOLDER', budgetHolderEmail);
    console.log('budgetHolderSubject: ', budgetHolderSubject)
    TravelRequestHelper.sendTravelRetirementEmail(currentTravelRequest, budgetHolderEmail, budgetHolderSubject);
  }
  ...
  if (isFINANCE || nextFINANCE) {
    ...
    if (nextFINANCE) {
      // Send to NEXT USER APPROVAL
      const { otherUserEmails, assignedFullName } = fetchOtherUsers(financeApprover, financeCond)
      console.log(`FINANCE approval assigned to: ${assignedFullName}, then inform other bst members`, otherUserEmails);
      otherUserSubject = otherUserSubject + createdBy.profile.fullName + "has been assigned to " + assignedFullName;
      TravelRequestHelper.sendTravelRetirementEmail(currentTravelRequest, otherUserEmails, otherUserSubject);
    }
  }
  ...
  if (isBST || nextBST) {
    console.log('Email To: BST', bstEmail);
    console.log('bstSubject: ', bstSubject)
    TravelRequestHelper.sendTravelRetirementEmail(currentTravelRequest, bstEmail, bstSubject);
  }
}
 ...
 * @param {*} nextUserID 
 */
 Core.sendUpdateOrCancellationMail = (currentTravelRequest, TravelRequestHelper, isUpdated) => {
  let otherPartiesEmail = "bulkpay@c2gconsulting.com" + supportmail ? ` ${supportmail}, ` : supportmail;
  const defaultUserData = { profile: {} };
  ...
  let otherUserSubject = "Travel request process for ";
  let bookingAgentSubject = "Ticket booking cancellation: Travel Request for " + createdBy.profile.fullName + " has been cancelled by the Administrator";
  let securityDeptSubject = "Travel Request for: " + createdBy.profile.fullName + " has been cancelled by the Administrator";
  let createdBySubject = "Travel Request for: " + createdBy.profile.fullName + " has been cancelled by the Administrator";
  let supervisorSubject = "Travel Request for: " + createdBy.profile.fullName + " has been cancelled by the Administrator";
  let budgetHolderSubject = "Travel Request for: " + createdBy.profile.fullName + " has been cancelled by the Administrator";
  let managerSubject = "Travel Request for: " + createdBy.profile.fullName + " has been cancelled by the Administrator";
  let gcooSubject = "Travel Request for: " + createdBy.profile.fullName + " has been cancelled by the Administrator";
  let gceoSubject = "Travel Request for: " + createdBy.profile.fullName + " has been cancelled by the Administrator";
  let bstSubject = "Travel Request for: " + createdBy.profile.fullName + " has been cancelled by the Administrator";
  let logisticsSubject = "Travel Request for: " + createdBy.profile.fullName + " has been cancelled by the Administrator";

  if (isUpdated) {
    bookingAgentSubject = "Please Update Ticket for " + createdBy.profile.fullName;
    securityDeptSubject = "Please approve the updated travel request for " + createdBy.profile.fullName;
    createdBySubject = "Updated travel request for: " + createdBy.profile.fullName;
    supervisorSubject = "Please approve the updated travel request for " + createdBy.profile.fullName;
    budgetHolderSubject = "Please approve the updated travel request for " + createdBy.profile.fullName;
    managerSubject = "Please approve the updated travel request for " + createdBy.profile.fullName;
    gcooSubject = "Please approve the updated travel request for " + createdBy.profile.fullName;
    gceoSubject = "Please approve the updated travel request for " + createdBy.profile.fullName;
    bstSubject = "Please approve the updated travel request for " + createdBy.profile.fullName;
    logisticsSubject = "Please approve the updated travel request for " + createdBy.profile.fullName;
  }
  ...
  //Send to Supervisor
  if (currentTravelRequest.status === APPROVED_BY_HOD || currentTravelRequest.status === REJECTED_BY_HOD) {
    console.log('Email To: HOD', supervisorEmail);
    console.log('supervisorSubject: ', supervisorSubject)
    TravelRequestHelper.sendTravelRequestEmail(currentTravelRequest, supervisorEmail, supervisorSubject);
  }
  ...
  //Send to Manager
  if (currentTravelRequest.status === APPROVED_BY_MD || currentTravelRequest.status === REJECTED_BY_MD) {
    console.log('Email To: MD', managerEmail);
    console.log('managerSubject: ', managerSubject)
    TravelRequestHelper.sendTravelRequestEmail(currentTravelRequest, managerEmail, managerSubject);
  }
  ...
  //Send to GCOO
  if (currentTravelRequest.status === APPROVED_BY_GCOO || currentTravelRequest.status === REJECTED_BY_GCOO) {
    console.log('Email To: GCOO', gcooEmail);
    console.log('gcooSubject: ', gcooSubject)
    TravelRequestHelper.sendTravelRequestEmail(currentTravelRequest, gcooEmail, gcooSubject);
  }
  ...
  //Send to GCEO
  if (currentTravelRequest.status === APPROVED_BY_GCEO || currentTravelRequest.status === REJECTED_BY_GCEO) {
    console.log('Email To: GCEO', gceoEmail);
    console.log('gceoSubject: ', gceoSubject)
    TravelRequestHelper.sendTravelRequestEmail(currentTravelRequest, gceoEmail, gceoSubject);
  }
 ...
 */
 Core.sendRetirementUpdateMail = (currentTravelRequest, TravelRequestHelper) => {
  ...
  let otherUserSubject = "Travel retirement process for ";
  let bookingAgentSubject = "Please Update Ticket for " + createdBy.profile.fullName;
  let securityDeptSubject = "Please approve the updated travel retirement for " + createdBy.profile.fullName;
  let createdBySubject = "Updated travel retirement for: " + createdBy.profile.fullName;
  let supervisorSubject = "Please approve the updated travel retirement for " + createdBy.profile.fullName;
  let budgetHolderSubject = "Please approve the updated travel retirement for " + createdBy.profile.fullName;
  let managerSubject = "Please approve the updated travel retirement for " + createdBy.profile.fullName;
  let gcooSubject = "Please approve the updated travel retirement for " + createdBy.profile.fullName;
  let gceoSubject = "Please approve the updated travel retirement for " + createdBy.profile.fullName;
  let bstSubject = "Please approve the updated travel retirement for " + createdBy.profile.fullName;
  let logisticsSubject = "Please approve the updated travel retirement for " + createdBy.profile.fullName;
  ...
  //Send to Supervisor
  if (currentTravelRequest.retirementStatus === RETIREMENT_APPROVED_BY_HOD || currentTravelRequest.retirementStatus === RETIREMENT_REJECTED_BY_HOD) {
    console.log('Email To: HOD', supervisorEmail);
    console.log('supervisorSubject: ', supervisorSubject)
    TravelRequestHelper.sendTravelRetirementEmail(currentTravelRequest, supervisorEmail, supervisorSubject);
  }
  ...
  //Send to Manager
  if (currentTravelRequest.retirementStatus === RETIREMENT_APPROVED_BY_FINANCE || currentTravelRequest.retirementStatus === RETIREMENT_REJECTED_BY_FINANCE) {
    console.log('Email To: FINANCE', financeApproverEmail);
    console.log('financeApproverSubject: ', financeApproverSubject)
    TravelRequestHelper.sendTravelRetirementEmail(currentTravelRequest, financeApproverEmail, financeApproverSubject);
  ...
  //Send to Supervisor
  if (currentTravelRequest.retirementStatus === RETIREMENT_APPROVED_BY_BST || currentTravelRequest.retirementStatus === RETIREMENT_REJECTED_BY_BST) {
    console.log('Email To: BST', bstEmail);
    console.log('bstSubject: ', bstSubject)
    TravelRequestHelper.sendTravelRetirementEmail(currentTravelRequest, bstEmail, bstSubject);
