...
GeoCoder = function geoCoderConstructor(options) {
  ...
  if (tenantSettings) {
    if (tenantSettings.settings.google) {
      extra = {
        clientId: tenantSettings.settings.google.clientId,
        apiKey: tenantSettings.settings.google.apiKey
      };
    }
  }*/
...
GeoCoder.prototype.geoip = function geoCoderGeocode(address, callback) {
  ...
  if (geoCallback) {
    ...
    try {
      ...
      Core.Log.warn("tenant/getLocale geoip lookup failure", error);
      return {};
    }
  }
};
