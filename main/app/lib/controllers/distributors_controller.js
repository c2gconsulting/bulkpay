DistributorsController = ApplicationController.extend({
  ...
  waitOn: function () {
    return CustomerSubs.subscribe('Customer', this.params._id);
  },
  ...
  detail: function () {
    ...
    if (customer && Core.hasCustomerAccess(Meteor.userId(), customer.groupCode, true)) {
      this.render('DistributorsDetail');
    } else {
  ...
  create: function () {
    if (Core.hasCustomerAccess(Meteor.userId())) {
      this.render('DistributorsCreate');
    } else {
      Router.go('distributors.list');
    }
  },
