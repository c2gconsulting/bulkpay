...
'click [id=additionalSecurityComment]': function(e, tmpl) {
  e.preventDefault()
    ...
    self.autorun(function(){
        ...
        if (invokeReason){
            ...
            if(travelRequest2Sub.ready()) {
              ...
              individuals = _.map(individuals, function(individual) {
                return individual.id
              })
              individuals = _.filter(individuals, function (individual) {
                return individual !== undefined;
              })
              if (individuals && individuals.length) {
                // $('.ui.fluid.dropdown').dropdown('set selected',['Role1','Role2']);
                $('.ui.fluid.dropdown').dropdown('set selected', individuals);
              }
