...
/*****************************************************************************/
/* TravelRequisition2GCOODetail: Event Handlers */
/*****************************************************************************/
...
Template.TravelRequisition2GCOODetail.events({
    'click #approve': (e, tmpl) => {
        let gcooComment = $('[name=gcooComment]').val();
        ...
        //update gcooComment one last final time
        currentTravelRequest.gcooComment = $("#gcooComment").val();
        ...
        // if (currentTravelRequest.gcooComment ===""){
        //     fieldsAreValid = false;
        //     validationErrors += ": GCOO Comment cannot be empty";
        // }
        ...
        if (fieldsAreValid){
            Meteor.call('TRIPREQUEST/gcooApprovals', currentTravelRequest, 'GCOO', true, (err, res) => {
                if (res){
                    swal({
    ...
    'click #reject': (e, tmpl) => {
        ...
        if (currentTravelRequest.status == Core.ALL_TRAVEL_STATUS.REJECTED_BY_GCOO && currentTravelRequest.gcooComment ===""){
            fieldsAreValid = false;
            validationErrors += ": GCOO Comment cannot be empty";
        }
...
/*****************************************************************************/
/* TravelRequisition2GCOODetail: Helpers */
/*****************************************************************************/
Template.TravelRequisition2GCOODetail.helpers({
    ACTIVITY: () => 'activityId',
    ...
    canApprove() {
        const currentTravelRequest = Template.instance().currentTravelRequest.get();
        const { GCOO } = Core.Approvals;
        return Core.canApprove(GCOO, currentTravelRequest)
    },
...
/*****************************************************************************/
/* TravelRequisition2GCOODetail: Lifecycle Hooks */
/*****************************************************************************/
Template.TravelRequisition2GCOODetail.onCreated(function () {
...
Template.TravelRequisition2GCOODetail.onRendered(function () {
...
Template.TravelRequisition2GCOODetail.onDestroyed(function () {
});
