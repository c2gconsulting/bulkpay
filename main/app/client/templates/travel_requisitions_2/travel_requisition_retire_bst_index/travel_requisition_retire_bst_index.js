...
Template.TravelRequisition2BSTRetirementIndex.helpers({
    ...
    'numberOfPages': function() {
        let limit = Template.instance().NUMBER_PER_PAGE.get()
        const { bstRetirementCond } = Core.getTravelQueries()
        let totalNum = TravelRequisition2s.find(bstRetirementCond).count()
...
Template.TravelRequisition2BSTRetirementIndex.onCreated(function () {
    ...
    self.getTravelRequestsByBST = function(skip) {
        ...
        options.skip = skip
        const { bstRetirementCond } = Core.getTravelQueries()
        return TravelRequisition2s.find(bstRetirementCond, options);
    }
