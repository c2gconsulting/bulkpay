/*****************************************************************************/
/* SalesHistory: Event Handlers */
/*****************************************************************************/
import Ladda from 'ladda';
Template.SalesHistory.events({
    ...
    'click .excel': function(event, tmpl){
        ...
        Meteor.call("orders/getExportData", options, function(err, data){
            if(err){
                console.log(err);
                resetButton();
            } else {
                let exportData = {fields: fields, data: data};
                TDCExporter.exportAllData(exportData, "SalesHistoryReport");
                resetButton();
            }
        });
    },
...
/*****************************************************************************/
/* SalesHistory: Helpers */
/*****************************************************************************/
Template.SalesHistory.helpers({
    salesUsers: function() {
...
/*****************************************************************************/
/* SalesHistory: Lifecycle Hooks */
/*****************************************************************************/
Template.SalesHistory.onCreated(function () {
    let self = this;
    ...
    if (!Session.get('orderHistorySortBy')) {
        Session.set('orderHistorySortBy', 'issuedAt');
    ...
    self.autorun(function () {
        ...
        let subscription = self.subscribe('ManageOrders', options, limit, sort);
    ...
    self.orders = function() {
        ...
        let sort = {};
        sort[sortBy] = sortDirection;
        return Orders.find(options, {sort: sort, limit: limit});
    };
...
Template.SalesHistory.onDestroyed(function () {
...
function getOptions(filterConditions) {
    ...
    let paymentStatus = getPaymentStatus(filterConditions);
    let approvalStatus = getApprovalStatus(filterConditions);
    if (status.length > 0 ){
        options["status"] = {$in: status}
    }
    if (paymentStatus.length > 0 ){
        options["paymentStatus"] = {$in: paymentStatus}
    }

    if (approvalStatus.length > 0 ){
        options["approvalStatus"] = {$in: approvalStatus}
    }
...
function getApprovalStatus(filterConditions){
    let approvalStatus = [];
...
function getPaymentStatus(filterConditions){
    let paymentStatus = [];
