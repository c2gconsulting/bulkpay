...
Template.Sales.events({
    ...
    'click a[href="#sort"]': function (e, tmpl) {
        ...
        if (Session.get('salesSortBy') === key) Session.set('salesSortDirection', 0 - Number(Session.get('salesSortDirection'))); //reverse sort direction
        else Session.set('salesSortBy', key);
    }
});
...
Template.Sales.helpers({
    ...
    results: function() {
        let results = Template.instance().reportData.get('reportData');
        let sortDirection = Session.get('salesSortDirection');
        let sortBy = Session.get('salesSortBy');
        if (sortDirection === -1){
    ...
    sortedBy: function (key) {
        let sortIcon = Session.get('salesSortDirection') === 1 ? 'glyphicon glyphicon-sort-by-attributes' : 'glyphicon glyphicon-sort-by-attributes-alt';
        return Session.get('salesSortBy') === key ? sortIcon : '';
    }
...
Template.Sales.onCreated(function () {
    ...
    if (!Session.get('salesSortBy')) {
        Session.set('salesSortBy', 'date');
        Session.set('salesSortDirection', -1);
    }
    ...
    instance.autorun(function () {
        ...
        Meteor.call("orders/salesData", filterConditions, function(err, res){

            instance.reportData.set('reportData', res);
            var graphData = instance.getGraphData(res);

            var salesChartCanvas = document.getElementById('intelligenceSalesReportCanvas').getContext("2d");
