...
Template.ReturnOrder.events({
    ...
    "click .submit-btn": function (e, tmpl) {
        ...
        Template.instance().reportData.set("filterObject", queryParams)
        */
    },
    ...
    'click a[href="#sort"]': function (e, tmpl) {
        ...
        if (Session.get('salesReturnOrderSortBy') === key) Session.set('salesReturnOrderSortDirection', 0 - Number(Session.get('salesReturnOrderSortDirection'))); //reverse sort direction
        else Session.set('salesReturnOrderSortBy', key);
    }
});
...
Template.ReturnOrder.helpers({
    reportTypes: function() {
        return ["Customer", "Salesperson", "Location", "Product Variant"]
    },
    startDate: function () {
    ...
    results: function(){
        let results = Template.instance().reportData.get("results") || []
        let sortDirection = Session.get('salesReturnOrderSortDirection');
        let sortBy = Session.get('salesReturnOrderSortBy');
        if (sortDirection === -1){
    ...
    sortedBy: function (key) {
        let sortIcon = Session.get('salesReturnOrderSortDirection') === 1 ? 'glyphicon glyphicon-sort-by-attributes' : 'glyphicon glyphicon-sort-by-attributes-alt';
        return Session.get('salesReturnOrderSortBy') === key ? sortIcon : '';
    }
});

/*****************************************************************************/
/* ReturnOrder: Lifecycle Hooks */
/*****************************************************************************/
Template.ReturnOrder.onCreated(function () {
    let instance = this;
    instance.reportData = new ReactiveDict();

    if (!Session.get('salesReturnOrderSortBy')) {
        Session.set('salesReturnOrderSortBy', 'label');
        Session.set('salesReturnOrderSortDirection', -1);
    }
    ...
    instance.returnReportChart = undefined;
    ...
    instance.autorun(function() {
        ...
        Meteor.call('returnorders/getReportData', filterObject, function(err, res) {
            if(err){
                console.log(err)
            } else {
                ...
                var returnChartCanvas = document.getElementById('intelligenceReturnOrderReportCanvas').getContext("2d");
                ...
                if (!instance.returnReportChart) {
                  instance.returnReportChart = new Chart(returnChartCanvas).Line(returnChartData, returnChartOptions);
                } else {
                  instance.returnReportChart.destroy();
                  instance.returnReportChart = new Chart(returnChartCanvas).Line(returnChartData, returnChartOptions);
                }
...
Template.ReturnOrder.onRendered(function () {
    let self = this;
});

Template.ReturnOrder.onDestroyed(function () {
});


/*****************************************************************************/
/* ReturnSelectableOptions: Helpers */
/*****************************************************************************/
Template.ReturnSelectableOptions.helpers({
    optionsForReportType: function(selected){
        return Session.get("options")
    },
});
...
/* ReturnSelectableOptions: Helpers */
/*****************************************************************************/
Template.ReturnSelectableOptions.onCreated(function () {
    let instance = this;
...
Template.ReturnSelectableOptions.onRendered(function () {
    let self = this;
...
Template.ReturnSelectableOptions.events({
  'change select': function (e, tmpl) {
      let self = this;
