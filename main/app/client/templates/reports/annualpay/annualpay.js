...
let AnnualPayReportUtils = {};

AnnualPayReportUtils.getNumberTaxCodeHeaderForMonth = (taxAmountHeaders, monthCode) => {    
...
AnnualPayReportUtils.processAnnualTaxReportData = (serverRes, year) => {
    ...
    _.each(taxAmountHeaders, taxHeader => {
        ...
        if(foundMonth) {
            ...
            let numTaxCodes = AnnualPayReportUtils.getNumberTaxCodeHeaderForMonth(taxAmountHeaders, taxHeader.monthCode)
...
/*****************************************************************************/
/* AnnualPayReport: Event Handlers */
/*****************************************************************************/
Template.AnnualPayReport.events({
    'click .getResult': (e, tmpl) => {
        const year = $('[name="paymentPeriod.year"]').val();
        if(year) {
            Meteor.call('getAnnualPayResult', Session.get('context'), year, function(err, res){
    ...
    'click .excel': (e, tmpl) => {
        ...
        if(year) {
            ...
            Meteor.call('getAnnualPayResult', Session.get('context'), year, function(err, res){
                if(res && res.taxAmountHeaders && res.taxAmountHeaders.length > 0) {
                    AnnualPayReportUtils.processAnnualTaxReportData(res, year)
                    resetButton()
                } else {
                    console.log(err);
...
/*****************************************************************************/
/* AnnualPayReport: Helpers */
/*****************************************************************************/
Template.registerHelper('formatDate', function(date) {
  return moment(date).format('MMYYYY');
});

Template.AnnualPayReport.helpers({
    'tenant': function(){
...
/*****************************************************************************/
/* AnnualPayReport: Lifecycle Hooks */
/*****************************************************************************/
Template.AnnualPayReport.onCreated(function () {
    let self = this;
    self.dict = new ReactiveDict();

});

Template.AnnualPayReport.onRendered(function () {
    //$('#example').DataTable();
...
Template.AnnualPayReport.onDestroyed(function () {
});
